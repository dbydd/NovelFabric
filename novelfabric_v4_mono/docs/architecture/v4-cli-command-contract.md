...
Completed pi-evidence hardening is archived in `archive/v4-pi-evidence-loop-archive.md`; completed domain artifact materialization is archived in `archive/v4-domain-artifact-materialization-archive.md`. The next implementation cycle should close only the active gaps below:

| Priority | Command families                                | Gap                                                                                                                                                                                                   | Test standard                                                                                                                                               |
| -------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1        | `agent`, `runtime`, `web`                       | Wire the planned pi SDK `AgentSession` bridge with event streaming and Web-safe tool policy enforcement. The current CLI process bridge is accepted evidence, not the final nontechnical Web runtime. | Runtime/policy tests prove temp config roots, denied raw dangerous tools, stable event trace, and comparable task evidence envelopes.                       |
