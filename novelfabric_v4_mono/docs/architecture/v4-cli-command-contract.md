...
| `agent`                                   | pi CLI bridge implemented        | Task package exists; `agent run --runtime pi` launches a NovelFabric-owned pi CLI process with `generic-writer`, captures output, and writes audited result evidence. Full SDK AgentSession/Web binding remains pending. |
| `workflow`                                | pi evidence bridge implemented   | Job plan/start/step/status/retry/cancel/verify exist; pi-task stages run `agent run --runtime pi` and require completed runtime/output evidence. Full Web/SDK event stream and richer domain validation remain pending.  |
