...
| `agent`                                   | pi CLI bridge implemented        | Task package exists; `agent run --runtime pi` launches a NovelFabric-owned pi CLI process with `generic-writer`, validates output schema, captures runtime evidence, and writes audited result evidence. Full SDK AgentSession/Web binding remains pending.                                                                                                                                                                   |
| `workflow`                                | pi evidence bridge implemented   | Job plan/start/step/status/retry/cancel/verify exist; pi-task stages run `agent run --runtime pi`, require schema-valid output with source anchors, and verify hashed result evidence bound to the current job/stage. This validates agent task results only; materializing StorySwarm outputs, ReportAgent markdown, and chapter drafts as domain artifacts remains pending. Full Web/SDK event stream also remains pending. |
...
Completed pi-evidence hardening is archived in `archive/v4-pi-evidence-loop-archive.md`. The next implementation cycle should close only the active gaps below:

| Priority | Command families                                | Gap                                                                                                                                                                                                                                                                   | Test standard                                                                                                                                                                         |
| -------- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1        | `workflow`, `swarm`, `report`, `writing`        | Materialize validated pi `task/result.json` output into durable StorySwarm output, ReportAgent markdown/JSON, and writing draft/chapter artifacts. `workflow verify` should eventually require both validated pi task evidence and expected domain artifact evidence. | Service tests fail for missing/stale/hash-mismatched/domain-invalid artifacts even when pi task evidence exists; CLI tests prove JSON envelopes, audited writes, and artifact hashes. |
| 2        | `agent`, `runtime`, `web`                       | Wire the planned pi SDK `AgentSession` bridge with event streaming and Web-safe tool policy enforcement. The current CLI process bridge is accepted evidence, not the final nontechnical Web runtime.                                                                 | Runtime/policy tests prove temp config roots, denied raw dangerous tools, stable event trace, and comparable task evidence envelopes.                                                 |
| 3        | `web`, `workflow`                               | Expose the full browser workflow path from source upload/import through semantic拆书, cards/memory/timeline, StoryRAG/context, StorySwarm, ReportAgent, chapter generation, editor review, and save.                                                                  | Playwright UI-only test on 50000+ ports proves visible runtime evidence, final domain artifacts, editor content, and audit records.                                                   |
| 4        | `import`, `cards`, `memory`, `knowledge`        | Convert deterministic import outputs into pi-backed semantic materialization with content-quality validation and reversible/conflict-safe apply.                                                                                                                      | Multi-fixture tests prove generated assets cite source excerpts, differ by source content, reject low-quality output, and preserve source files on failure.                           |
| 5        | `external-swarm`                                | Land frozen REST/MCP adapters around the existing CLI compatibility artifacts.                                                                                                                                                                                        | Golden tests cover REST POST/GET, MCP tools/list/tools/call, idempotency, artifact path semantics, and `structuredContent`.                                                           |
| 6        | `cards`, `memory`, `swarm`, `report`, `writing` | Replace broad project/file write authority with narrower domain capabilities such as `swarm.run`, `report.write`, `writing.apply`, `memory.propose_shared`, and actor-scoped grants.                                                                                  | Capability tests prove main-agent success, role-agent denials, protected-path denial, and audit actor/capability/reason/hash fields.                                                  |
...
`agent run --runtime pi` currently uses the NovelFabric-owned pi CLI bridge with `generic-writer`; it must not route through a NovelFabric-owned provider adapter. The command must validate the task output schema, reject empty or schema-invalid output, record runtime config root, enabled extensions, tool policy, actor, task id, model/provider evidence, and output artifact paths. Durable outputs should land as proposals or workflow task results and be applied by domain commands. Full SDK `AgentSession` and Web-safe session streaming remain pending.
