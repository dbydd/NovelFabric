...
The completed pi-backed semantic evidence loop is archived in `archive/v4-pi-evidence-loop-archive.md`; completed domain artifact materialization is archived in `archive/v4-domain-artifact-materialization-archive.md`; completed opt-in SDK AgentSession execution is archived in `archive/v4-sdk-agent-session-opt-in-archive.md`; completed Web-safe read-only SDK tools foundation is archived in `archive/v4-web-safe-sdk-tools-foundation-archive.md`. Do not keep re-litigating completed hardening, materialization, opt-in SDK execution, or read-only SDK tool foundation details in the active plan. This section tracks only the gaps still blocking a complete product/business loop.

### 11.1 Active Gaps In Priority Order

| Priority | Gap                                                  | Required output                                                                                                                                                                           | Minimum test standard                                                                                                                                                                                                                                   |
| -------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1        | Mutation tools for Web-safe SDK sessions             | Add Web-safe mutation tools such as `novelfabric_write_file`, `novelfabric_apply_proposal`, and domain artifact apply tools on top of the archived read-only tool foundation.             | Tool tests prove namespace restrictions, actor/capability checks, base-hash conflict behavior, protected-path denial, audit JSONL, and no raw filesystem mutation; negative tests cover traversal, wrong namespace, stale hash, and unauthorized actor. |
| 2        | Web runtime event stream and lifecycle orchestration | Upgrade bridge runtime routes from sanitized task snapshots to browser-visible start/status/stream/cancel/retry lifecycle with bounded/redacted event traces.                             | Bridge tests prove streaming event shape, denial events, cancellation/retry cleanup, no internal path/session leakage, and stable evidence envelopes for completed/failed/cancelled runs.                                                               |
...
- Build on the archived opt-in `agent run --runtime pi-sdk` AgentSession path and archived read-only Web-safe SDK tools foundation rather than treating SDK execution or read-only tooling as absent.
- Implement mutation tools for write/apply flows (`novelfabric_write_file`, `novelfabric_apply_proposal`, domain artifact apply) with namespace, capability, base-hash, protected-path, and audit checks.
- Add Web bridge session lifecycle controls, real-time/bounded event streaming, cancellation, retry, and browser-visible runtime trace.
