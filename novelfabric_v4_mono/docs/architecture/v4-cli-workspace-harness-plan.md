...
- Build on the archived opt-in `agent run --runtime pi-sdk` AgentSession path rather than treating SDK execution as absent.
- Implement NovelFabric-controlled SDK extensions/tools for read/write/context/validate/apply/report, with raw dangerous tools denied for Web sessions.
- Add Web bridge session lifecycle controls, event streaming, cancellation, retry, and browser-visible runtime trace.
