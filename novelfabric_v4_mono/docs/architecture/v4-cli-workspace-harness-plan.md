...
- Build on the archived opt-in `agent run --runtime pi-sdk` AgentSession path, archived read-only Web-safe SDK tools foundation, and archived Web-safe mutation tools foundation rather than treating SDK execution or tool coverage as absent.
