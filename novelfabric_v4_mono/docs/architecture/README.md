...
2. `v4-canonical-resource-materialization-gap.md` — current active V4 gap definition for canonical card/memory/timeline/simulation/chapter completeness.
3. `v4-cli-command-contract.md` — target `novelfabric` CLI command surface, JSON envelopes, error codes, capabilities, and implementation status.
4. `../qa/v4-full-usability-acceptance.md` — full usability test contract and evidence standard.
5. `v4-mono-frontend-plan.md` / `v4-mono-frontend-plan.zh.md` — Web shell design and bridge integration rules.
...
The latest `test_novel.txt` real-path evidence is archived in `archive/v4-real-path-partial-coverage-2026-06-08.md`. That work proved the CLI workflow spine plus pi-backed domain artifacts, but it did **not** prove full canonical business completeness. `cards/rules`, `cards/world`, `cards/scenes`, multi-character card extraction, `memory/**`, `timeline/branches`, `simulation/turns`, `simulation/logs`, and `writing/chapters` remained empty or incomplete after the run. Active planning should therefore treat canonical project resource materialization as a fresh open gap rather than treating the archived workflow-spine smoke as product-complete.
