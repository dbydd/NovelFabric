...
已完成的 pi-backed semantic evidence loop 已归档到 `archive/v4-pi-evidence-loop-archive.md`，已完成的 domain artifact materialization 已归档到 `archive/v4-domain-artifact-materialization-archive.md`，已完成的 opt-in SDK AgentSession execution 已归档到 `archive/v4-sdk-agent-session-opt-in-archive.md`。active plan 不再重复堆叠已完成 hardening / materialization / opt-in SDK execution 项，只保留阻塞完整业务闭环的未完成 gap。

### 11.1 未完成 Gap 与测试标准

| 优先级 | Gap                                                             | 必须产出                                                                                                                                                                             | 最低测试标准                                                                                                                                                                                                                                                       |
| ------ | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1      | Web-safe runtime extensions 与 Web bridge session orchestration | 基于已归档的 opt-in SDK AgentSession path，补齐 NovelFabric-controlled SDK extensions/tools、runtime trace、Web bridge lifecycle controls 与 Web-safe policy enforcement。           | Runtime tests 用临时 config roots 实例化 SDK bridge；policy tests 证明 Web sessions 拒绝 raw `bash`/`write`/`edit`/network/arbitrary paths；extension tests 证明 NovelFabric tools 通过 CLI/shared services；event-stream tests 校验 browser-visible trace shape。 |
...
| 5      | Domain-specific capabilities                                    | cards/memory/swarm/report/writing 使用更窄 capability names，而不是 broad project/file write authority。                                                                             | Capability tests 证明 main agent 可执行授权 domain 操作，role agents 被拒绝跨域/保护操作，audit records 包含 actor/capability/reason。                                                                                                                             |
...
9. **Web-Safe pi SDK Runtime Bridge** — 基于已归档的 `agent run --runtime pi-sdk` opt-in AgentSession path，补齐 NovelFabric-controlled SDK extensions/tools、Web bridge session lifecycle controls、event streaming、cancellation/retry 与 browser-visible runtime trace；保持现有 agent task create/inspect/run/output validate/status/abort 命令契约，并继续记录 session/task evidence。
