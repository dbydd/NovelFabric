...
| 优先级 | Gap                                                 | 必须产出                                                                                                                                                                             | 最低测试标准                                                                                                                                                                                                                          |
| ------ | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1      | Web-safe SDK mutation tools                         | 基于已归档的 read-only SDK tools foundation，补齐 `novelfabric_write_file`、`novelfabric_apply_proposal`、domain artifact apply 等 Web-safe mutation tools。                         | Tool tests 证明 namespace restrictions、actor/capability checks、base-hash conflict、protected-path denial、audit JSONL、无 raw filesystem mutation；negative tests 覆盖 traversal、wrong namespace、stale hash、unauthorized actor。 |
| 2      | Web runtime event stream 与 lifecycle orchestration | 将 bridge runtime routes 从 sanitized task snapshots 升级为 browser-visible start/status/stream/cancel/retry lifecycle，并提供 bounded/redacted event trace。                        | Bridge tests 证明 streaming event shape、denial events、cancellation/retry cleanup、无 internal path/session 泄漏，以及 completed/failed/cancelled runs 的稳定 evidence envelope。                                                    |
...
9. **Web-Safe pi SDK Runtime Bridge** — 基于已归档的 `agent run --runtime pi-sdk` opt-in AgentSession path 与 read-only SDK tools foundation，补齐 Web-safe mutation tools（write/apply/domain artifact apply）、Web bridge session lifecycle controls、real-time/bounded event streaming、cancellation/retry 与 browser-visible runtime trace；保持现有 agent task create/inspect/run/output validate/status/abort 命令契约，并继续记录 session/task evidence。
