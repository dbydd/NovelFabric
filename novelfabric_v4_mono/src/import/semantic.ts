...
export type SemanticImportTaskResult = {
  ...
  readonly requiredSourceAnchors: readonly string[];
...
export async function createSemanticImportTask(
  ...
  const requiredSourceAnchors = deriveSemanticSourceAnchors(
    sourceRead.content,
    contextRead.content
  );
  const task = await createAgentTask({
    ...
    instruction: [
      ...
      "sourceAnchors must contain every requiredSourceAnchors item exactly as written; do not summarize, translate, shorten, or invent anchors.",
      `Required source anchors: ${requiredSourceAnchors.join(", ")}`,
    ...
    outputSchemaJson: stableJson(
      semanticImportOutputSchema(requiredSourceAnchors, request.sourcePath, request.contextPackPath)
    ),
...
function semanticImportOutputSchema(
...
function deriveSemanticSourceAnchors(
  sourceText: string,
  contextContent: string
): readonly string[] {
  const context = parseOptionalJsonObject(contextContent);
  const anchors = new Set<string>();
  const add = (value: unknown): void => {
    if (typeof value !== "string") return;
    const text = value.trim();
    if (text.length >= 2 && sourceText.includes(text)) anchors.add(text);
  };
  if (context !== null) {
    const chapters = context["chapters"];
    if (Array.isArray(chapters)) {
      for (const chapter of chapters) {
        if (isRecord(chapter)) add(chapter["title"]);
      }
    }
    const excerpt = context["sourceExcerpt"];
    if (typeof excerpt === "string") {
      for (const part of splitAnchorCandidates(excerpt)) add(part);
    }
  }
  for (const part of splitAnchorCandidates(sourceText)) add(part);
  const selected = [...anchors].slice(0, 6);
  if (selected.length < 2) {
    throw new CommandFailure(
      "semantic_import_source_anchors_missing",
      "Semantic import requires at least two exact source anchors from the source text."
    );
  }
  return selected;
}

function splitAnchorCandidates(text: string): readonly string[] {
  return text
    .split(/[\r\n，。,.!?！？；;：:]+/u)
    .map((item) => item.trim())
...
function parseOptionalJsonObject(content: string): JsonObject | null {
  try {
