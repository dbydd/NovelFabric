# NovelFabric V4 工作区风格后端 / Mono App 规划案

> 状态：V4 已进入实际构建。`novelfabric_v4_mono` 目前包含从旧 `backend_v2` staging 迁移而来的 TypeScript CLI foundation 与可选 Web shell layout foundation。
>
> 范围：把 NovelFabric 改造成可以被 pi / Hermes 这类 agent 直接接管的文本工作区。后端收缩为一组小型 TypeScript CLI 原语、同包可选 Vue Web shell 与薄 pi/local bridge；人物推理、人物调度、KP/世界维护/项目审核等智能工作移交给外部 agent 与 skill。
...
V4 实现已从旧 staging 目录 `backend_v2/` 改名到 `novelfabric_v4_mono/`，并使用 TypeScript。本文旧有 Rust 表述属于历史迁移语境，应理解为共享 service / CLI 原语意图，不再作为在 V4 TypeScript 目录新增 Cargo crate 的指令。
...
- 不把 `novelfabric_v4_mono` 做成通用 PTY/session 管理器；pi/Hermes 已经拥有这层。
...
V4 Web surface 现在作为同一个 TypeScript 包内的可选 mono app shell 演进。Vue 前端在 V4 不再调用后端 LLM endpoint。
...
当前 layout-only demo 通过 `novelfabric web demo` / Vite scripts 显式启动，只使用静态 mock 数据。后续可选 HTTP bridge 可以提供：
...
1. `novelfabric_v4_mono` 等价 CLI 覆盖完成前，不改旧文件。
...
- `novelfabric_v4_mono/novelfabric_v2backend_workspace_style_backend.md`
- `novelfabric_v4_mono/novelfabric_v2backend_workspace_style_backend.zh.md`
- 尚未实现运行时代码
...
- `novelfabric_v4_mono/package.json`、`package-lock.json`、`tsconfig.json`、`tsconfig.build.json` 与严格 lint/test 配置
...
5. **旧 API 兼容**
   ...
   现有前端和测试依赖 HTTP endpoint。V4 应在 `novelfabric_v4_mono` staging 完成后再切换，或保留兼容 bridge。
...
规划阶段接受后，继续在 `novelfabric_v4_mono/` 开发：
...
`novelfabric_v4_mono` 已不再是纯规划目录；它包含 V4 TypeScript runtime code 与 layout-only Web shell。
