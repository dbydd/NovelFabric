...
describe("deterministic import services", () => {
  ...
  it("chapterizes Chinese headings and falls back to one chapter for headingless text", async () => {
    const withHeadings =
      "序章 风声\n雾从城门进来。\n第一章 星门\n少女醒来。\n第二章 雨城\n灯火摇晃。\n";
    await fs.writeFile(
      path.join(workspacePath, "imports/source/chapters.txt"),
      withHeadings,
      "utf8"
    );
    ...
    const chapterized = await chapterizeImportSource({
      workspacePath,
      actor: "main_agent",
      sourcePath: "imports/source/chapters.txt"
    });

    expect(chapterized.fallback).toBe(false);
    expect(chapterized.chapters.map((chapter) => chapter.title)).toEqual([
      "序章 风声",
      "第一章 星门",
      "第二章 雨城"
    ]);
    expect(chapterized.manifestPath).toBe("imports/chapters/chapters/manifest.json");

    const headingless = "没有章节标题的短篇。\n它也应该成为一个章节。\n";
    await fs.writeFile(
      path.join(workspacePath, "imports/source/headingless.txt"),
      headingless,
      "utf8"
    );
    const fallback = await chapterizeImportSource({
      workspacePath,
      actor: "main_agent",
      sourcePath: "imports/source/headingless.txt"
    });
    expect(fallback.fallback).toBe(true);
    expect(fallback.chapters).toHaveLength(1);
    expect(fallback.chapters[0]?.title).toBe("headingless");
  });
  ...
  it("builds context packs, lists inbox artifacts, and validates import health", async () => {
    ...
    expect(contextPack.chapterCount).toBe(2);
    ...
    expect(packContent.chapters).toHaveLength(2);
    ...
    expect(
      inbox.chapterManifests.some(
        (item) => item.path === "imports/chapters/contextual/manifest.json"
      )
    ).toBe(true);
    expect(
