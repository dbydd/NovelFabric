...
> Status: V4 construction active. `novelfabric_v4_mono` now contains the TypeScript CLI foundation and optional web-shell layout foundation that began under the old `backend_v2` staging directory.
>
...
The V4 implementation is TypeScript. Its staging directory has been renamed from `backend_v2/` to `novelfabric_v4_mono/`. Older Rust planning language in this document is historical migration context and should be interpreted as shared-service/CLI intent, not as an instruction to add Cargo crates in the V4 TypeScript directory.
...
V4 initializes `novelfabric_v4_mono` as a TypeScript workspace with one shared service layer and one preferred user-facing `novelfabric` CLI entry. Use capability-scoped subcommands rather than many unrelated binaries; subcommands should stay thin wrappers over shared TypeScript services. Skill-facing verbs should be coarse and stable so skill contracts do not become brittle.
...
1. Keep old files untouched until `novelfabric_v4_mono` has equivalent CLI coverage.
...
- `novelfabric_v4_mono/AGENTS.md`
- this planning document
- no runtime code yet

Verification:

- files exist under `novelfabric_v4_mono`
...
- `novelfabric_v4_mono/package.json`, `package-lock.json`, `tsconfig.json`, `tsconfig.build.json`, and strict lint/test configuration
...
5. **Old API compatibility**
   The existing frontend and tests expect HTTP endpoints. V4 should avoid breaking them until the new web bridge is ready, or should stage the work under `novelfabric_v4_mono` only.
...
After this planning stage is accepted, continue development in `novelfabric_v4_mono/`:

1. Continue from the TypeScript CLI foundation already committed in `novelfabric_v4_mono`.
