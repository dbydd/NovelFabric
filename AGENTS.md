...
已完成的 pi-backed semantic evidence loop、domain artifact materialization、opt-in SDK AgentSession execution、Web-safe read-only SDK tools foundation 与 Web-safe mutation tools foundation 已归档到：
...
1. 补齐 Web runtime event stream 与 lifecycle orchestration：browser-visible start/status/stream/cancel/retry、bounded/redacted runtime events、denial trace 与稳定 evidence envelope。
