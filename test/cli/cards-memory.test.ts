...
describe("cards and memory command registrations", () => {
  ...
  it("keeps semantic resource filling out of deterministic CLI command registration", async () => {
    const proposed = await runRegisteredCommand([
      "cards",
      "propose",
      "--workspace",
      ...
      "雨城入口",
      "--target-path",
      "cards/scenes/rain-city-entry.md",
      "--content",
      "# 雨城入口\n\n通过显式 proposal 写入。\n",
      "--json"
    ]);
    expect(proposed.command).toBe("cards propose");
    expect(proposed.data.cardCount).toBe(1);

    const program = new Command();
    addTimelineCommands(program);
    const timeline = program.commands.find((command) => command.name() === "timeline");
    expect(timeline).toBeDefined();
    expect(timeline?.commands.map((command) => command.name()) ?? []).not.toContain("materialize");
  });
