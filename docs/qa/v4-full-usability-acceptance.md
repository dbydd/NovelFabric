...
## 8. Fresh Active Gap: Agent-Generated Workspace Resource Filling
...
Observed failures from the previous direction:

- semantic workspace resources were being pushed toward deterministic TypeScript projection instead of constrained model/agent generation;
- this blurred the boundary between tooling code and agent-owned semantic work;
- success criteria drifted toward "the code filled the files" instead of "the model/user produced correct files through NovelFabric guardrails".

This gap must stay open until a real-path run produces canonical workspace resources with substantive content, source citations, and model-run evidence. A passing `workflow verify` alone is not sufficient evidence for product completeness.

Representative acceptance tests for closing this gap:

- project/workflow paths create the required directory structure;
- model-backed task stages or user proposals populate the relevant card/memory/timeline/simulation/chapter files;
- populated files contain the correct resource type, required evidence, and proposal/task provenance;
- tests prove the semantic content came from the NovelFabric-wrapped pi/model path rather than deterministic extraction logic in the core CLI;
- Playwright and CLI acceptance check workspace completeness and audit evidence, not only job/task status.
...
- semantic workspace content is traceable to a model-backed task result or user proposal rather than deterministic core-code extraction;
