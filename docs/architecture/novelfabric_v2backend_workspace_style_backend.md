# NovelFabric V4 Workspace-Style Backend / Mono App Plan

> Status: V4 construction active. The repository root now contains the TypeScript CLI foundation and optional web-shell layout foundation that began under the old `backend_v2` / `novelfabric_v4_mono` staging directories.
>
> Scope: redesign NovelFabric so a project can be directly taken over by pi / Hermes style agents. The backend becomes a set of small TypeScript CLI primitives plus an optional same-package Vue web shell and thin pi/local bridge, while character reasoning and scheduling move to external agents and skills.
...
The V4 implementation is TypeScript. Its active code now lives at the repository root after graduating from the old `backend_v2/` and `novelfabric_v4_mono/` staging directories. Older Rust planning language in this document is historical migration context and should be interpreted as shared-service/CLI intent, not as an instruction to add Cargo crates in the V4 TypeScript codebase.
...
V4 initializes the repository root as a TypeScript workspace with one shared service layer and one preferred user-facing `novelfabric` CLI entry. Use capability-scoped subcommands rather than many unrelated binaries; subcommands should stay thin wrappers over shared TypeScript services. Skill-facing verbs should be coarse and stable so skill contracts do not become brittle.
...
The V4 web surface now lives in the same TypeScript package as an optional mono app shell. The Vue frontend should not call backend LLM endpoints in V4.
...
The current layout-only demo is started explicitly through `novelfabric web demo` / Vite scripts and uses static mock data only. The optional HTTP bridge may later serve:
...
1. Keep old files untouched until the root-level V4 mono app has equivalent CLI coverage.
...
- archived staging AGENTS plus this planning document
- no runtime code yet at the time this phase was written

Verification:

- planning artifacts exist and document explicitly lists split functions, deprecated paths, and phases

### Phase V4.1 - TypeScript Workspace Skeleton

Deliverables:

- root `package.json`, `package-lock.json`, `tsconfig.json`, `tsconfig.build.json`, and strict lint/test configuration
...
5. **Old API compatibility**
   Some historical frontend paths and tests expected HTTP endpoints. V4 should avoid breaking compatibility until the new web bridge is ready.
...
After this planning stage is accepted, continue development in the root-level V4 mono app:

1. Continue from the TypeScript CLI foundation now living at the repository root.
...
The old staging directory is no longer the active implementation location; the repository root is.
