...
- The mono app wraps the pi agent SDK as its LLM runtime. For nontechnical Web users, the wrapped runtime must use NovelFabric-owned config paths under `~/.config/novelfabric/pi/`, NovelFabric-controlled extensions (sandbox/path-guard/permission-gate/CLI-only-write-tools), and must default-deny raw `bash`, raw `write`, raw `edit`, arbitrary network, and arbitrary path access.
...
- If a rule can be expressed in `AGENTS.md`, workspace skills, task prompts, output schemas, or capability manifests, do not hard-code it as TypeScript story logic.
- Core TypeScript may create directories, package tasks, validate schemas/citations/hashes, apply proposals, and audit writes. It must not become a deterministic novel-understanding engine that classifies cards, synthesizes memory, or infers timelines in place of constrained model/agent work.
- When Web UI needs state/orchestration code that is not suitable for the core harness, keep that logic under `src/web/` or Web bridge adapters rather than the shared runtime/business core.
...
Semantic-resource acceptance must be split in two:

1. deterministic harness checks: required directories/files exist, writes are audited, CLI/Web routes respect capability and path guards;
2. model-backed business checks: content is present in the right files, content is source-cited and proposal/task-backed, and evidence proves it was generated by the constrained runtime rather than deterministic extraction logic.
