...
| root `AGENTS.md` and `archive/v4-mono-staging-agents.md` | Binding current project constraints plus archived staging constraints when historical context matters.        |
...
7. **Skills before code branches** — If behavior can be expressed as an agent instruction, skill, role profile, soul, prompt template, output schema, or capability rule, it belongs in text assets rather than hidden TypeScript control flow.
8. **No coded story understanding** — Core TypeScript must not classify source text into cards, synthesize memories, infer timelines, or judge story semantics when that work can be performed by a constrained model/agent task.
9. **Proposal before apply** — Agent/user outputs become proposals or task artifacts first. CLI validators decide whether they can be applied to canonical files.
10. **Derived indexes only** — StoryGraph / StoryRAG artifacts can always be rebuilt from workspace source files.
...
There are currently **no open gaps from the previous V4 next-iteration ledger**. The current open gap is a scope reset: canonical workspace resources must be filled by constrained model/agent output, not by deterministic TypeScript story-materialization code. New work must create or update a fresh gap entry before implementation, including:

- the affected CLI/Web/runtime families;
- expected workspace directories, files, proposal paths, task evidence, and audit paths;
- acceptance tests that validate content correctness and model-generation evidence, not prose quality;
- reviewer/verifier criteria for archiving the gap once complete.

## 12. Fresh Active Gap: Agent-Generated Workspace Resource Filling

Fresh active gap after the V4 scope reset.

- **Gap name:** Agent-generated workspace resource filling and validation.
- **Observable failure:** previous work tried to close canonical resource gaps by adding deterministic TypeScript projections from semantic import artifacts into cards, memory, timeline, simulation logs, and chapters. That places semantic novel-understanding in core code instead of in `AGENTS.md`, skills, prompts, and model-generated proposals.
...
- **Acceptance tests required before closing:**
  1. project/workflow paths create the required directory skeleton;
  2. relevant directories contain files after model-backed task stages complete;
  3. file content is the correct resource type, substantive, cited, and traceable to a proposal/task result;
  4. tests prove semantic content came from the NovelFabric-wrapped pi/model path or user proposal, not deterministic keyword/template extraction;
  5. deterministic stages are labeled as harness/tooling only;
  6. Web and CLI acceptance check workspace completeness and audit paths, not only job/task completion.
- **Reviewer/verifier archival criteria:** close this gap only after a real-path run proves resource files, task evidence, proposal/apply audit, and absence of coded story-materialization shortcuts in the core CLI.
...
### Phase 4 — Import / Semantic Extraction CLI

- Add inbox, normalize, chunk, context-pack, validate, and pi-backed semantic extraction.
...
- Proposals may be written by users or constrained model/agent tasks; core TypeScript must not synthesize story content as a substitute for those proposals.
...
- canonical workspace resources were materially populated by model/user proposals, including at least one `cards/world`, `cards/rules`, `cards/scenes`, memory, and timeline path when the agent task says those resource types should exist;
- writing reached canonical chapter apply from a validated model/user draft instead of remaining in task-only evidence;
- tests can distinguish model-generated semantic content from deterministic source-text projection;
