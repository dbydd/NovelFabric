...
> 状态：V4 已进入实际构建。仓库根目录目前承载从旧 `backend_v2` / `novelfabric_v4_mono` staging 迁移而来的 TypeScript CLI foundation 与可选 Web shell layout foundation。
>
...
V4 实现已经从旧 staging 目录 `backend_v2/` / `novelfabric_v4_mono/` 翻转到仓库根目录，并使用 TypeScript。本文旧有 Rust 表述属于历史迁移语境，应理解为共享 service / CLI 原语意图，不再作为在 V4 TypeScript 代码库新增 Cargo crate 的指令。
...
- 不把根目录 V4 mono app 做成通用 PTY/session 管理器；pi/Hermes 已经拥有这层。
...
1. 根目录 V4 mono app 完成等价 CLI 覆盖前，不改旧文件。
...
- 根目录 `package.json`、`package-lock.json`、`tsconfig.json`、`tsconfig.build.json` 与严格 lint/test 配置
...
5. **旧 API 兼容**
   ...
   一些历史前端路径和测试依赖 HTTP endpoint。V4 应在新 Web bridge 就绪前保持兼容。
...
旧 staging 目录已不再是当前实现位置；当前 V4 TypeScript runtime code 与 Web shell 位于仓库根目录。
