...
| 根目录 `AGENTS.md` 与 `archive/v4-mono-staging-agents.md` | 当前项目约束，以及在需要历史语境时参考的 staging 约束归档。                                   |
...
4. **Import / Semantic Extraction CLI** — inbox、normalize、chunk、context-pack、validate；semantic extraction 由 pi skill 输出并由 CLI / downstream domain apply 消费。
