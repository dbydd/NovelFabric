...
export async function buildSimulationContextPack(
  ...
  const anchorSource = await simulationAnchorSource(request.workspacePath, session);
  const payload = {
    ...
    relevantEntities: normalizeRelevantEntities(request.relevantEntities ?? anchorSource.anchors),
  ...
  if (sourcePath !== null) {
    try {
      ...
      throw new CommandFailure(
        ...
        `Simulation session references source '${sourcePath}', but it could not be read: ${detail}`
      );
    }
  }
  return {
    excerpt: session.objective.slice(0, SIMULATION_CONTEXT_SOURCE_EXCERPT_MAX_CHARS),
    anchors: []
  };
}
...
function parseSession(content: string, artifactPath: string): SimulationSession {
  ...
  return {
    ...
    sourcePath: optionalString(parsed, "sourcePath"),
...
function optionalString(record: Record<string, unknown>, key: string): string | null {
  ...
  throw new CommandFailure(
    "invalid_artifact",
    `Artifact field '${key}' must be a string when present.`
  );
}
...
function normalizeRelevantEntities(values: unknown): readonly string[] {
  if (!Array.isArray(values)) return [];
  return [...new Set(values.filter((value): value is string => typeof value === "string").map((value) => value.trim()).filter((value) => value.length >= 2))].slice(
    0,
    SIMULATION_CONTEXT_SOURCE_ANCHOR_MAX_ITEMS
  );
}
