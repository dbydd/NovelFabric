...
export type KnowledgeGraphNodeKind = "document" | "episode";
...
export type KnowledgeGraphEdgeKind = "contains";
...
function buildEpisodesForDocument(document: SourceDocument): readonly KnowledgeGraphEpisode[] {
  ...
  const citation: KnowledgeCitation = {
    sourcePath: document.path,
    hash: document.hash,
    lineRange: { start: 1, end: countLines(document.content) },
    excerpt
  };
  return [
    {
      id: episodeNodeId(document.path, 0, document.content.length, document.title),
      sourcePath: document.path,
      title: document.title,
      ordinal: 1,
      charRange: { start: 0, end: document.content.length },
      lineRange: { start: 1, end: countLines(document.content) },
